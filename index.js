...
async function getTotalCommits(requests, contributor, cutoffDate) {
  const reposSub = await Promise.all(requests.map(p=>p.catch(e=>e)));
  const repos = reposSub.filter(result=>!(result instanceof Error))
...
async function main() {
  ...
  const colors = ["fdfefe", "fcfd54", "ea8d49", "8d503b", "262136"];
